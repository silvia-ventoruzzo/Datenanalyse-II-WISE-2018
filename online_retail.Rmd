...
customers_cluster_share <- purchases_first %>%
  filter(product_quantity > 0) %>% # Just in case
  group_by(customer_id) %>%
  mutate(total_products = n()*sum(product_quantity)) %>%
  group_by(customer_id, product_cluster) %>%
  summarize(share = n()*sum(product_quantity)/unique(total_products)) %>%
  ungroup() %>%
...
products_ordered_amount <- purchases %>%
  group_by(product_id, product_name) %>%
  summarize(times_ordered = n()*sum(product_quantity)) %>%
  ungroup() %>%
...
customers_category_share <- purchases %>%
...
customers <- purchases %>%
  ...
  left_join(customers_product_category, by = "customer_id") %>%
  replace(is.na(.), 0) %>%
  arrange(customer_id)
...
rm(customers_product_count, customers_cluster_share, )
```